...
| **MCP 工具调用** | 代码执行、网页搜索、天气查询、商店模拟、知识库、社交系统 |
...
│   │   ├── package.json
│   │   └── vite.config.ts
│   ├── mcp-servers/             # MCP Server 集合（工具调用）
│   │   ├── code-executor/       # Python 代码沙箱执行
│   │   ├── web-search/          # 网页搜索（Tavily）
│   │   ├── weather/             # 天气查询（OpenWeatherMap）
│   │   ├── shop-simulator/      # 商店模拟（小镇经济）
│   │   ├── knowledge-base/      # 小镇设定库
│   │   └── character-social/    # 角色社交（送礼/约会）
...
7. **[module-system.md](module-system.md)**：模块管理器与 MCP
...
MCP（Model Context Protocol）是角色调用外部工具的标准化协议。开发一个新的 MCP Server：

1. 在 `packages/mcp-servers/` 下创建新目录，如 `my-tool/`
2. 创建 `pyproject.toml` 和 `server.py`
3. 参考现有的 `code-executor/server.py` 实现 MCP 协议
4. 在 `.env` 中添加 `MCP_MY_TOOL_SERVER=http://localhost:8007`
5. 重启后端，通过 `GET /api/v1/mcp/servers` 验证
